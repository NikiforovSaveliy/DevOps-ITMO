...
Кроме того пользователю с помощью простых инструментов может настроить и имеет полный доступ к ним. В EC2 возможно использвоание многих вариантов инстансов. Одно из предлагамых опций - инстансы с eGPU, которые позволяют использовать гарфические ускорители для работы

1. DedicatedUsage - машинки, которые можно арендовать и использовать как VPS
2. LoadBalacer - Позволяет распределять тарфик по инстансам EC2, которые используются для работы
3. Elastic ip - предоставляет статический IP для динамичных облачных вычислений
4. CloudWatch - это служба мониторинга, которая предоставляет информацию и уведомления о работе экземпляров Amazon EC2 и ресурсов, связанных с ними. Она позволяет отслеживать метрики производительности, например, загрузку центрального процессора (CPU), использование памяти и сетевой трафик, а также настраивать оповещения и создавать системные журналы событий.

**AmazonDAX** - это сервис кеширования созданный для Amazon DynamoDb. DAX обеспечивает повышение производительности.
DAX ускоряет работу с таблицами DynamoDb. Сервис управляет аннулированием кэша, заполнением данных или управляет кластером. NodeUsage - это служба в Amazon (DAX), которая предоставляет информацию о использовании узлов DAX. 
...
1. AWS Device Farm WindowsChrome - это сервис от Amazon Web Services (AWS), который предоставляет возможность тестирования веб-приложений на различных версиях браузера Google Chrome под управлением операционной системы Windows.
2. AWS Device Farm IOS-device — это сервис, предлагаемый Amazon Web Services (AWS), который позволяет разработчикам тестировать и отлаживать свои приложения на устройствах с операционной системой iOS.
3. AWS Device Farm Android-RA - это составляющая Device Farm, которая обеспечивает удаленный доступ (Remote Access) к Android-устройствам, на которых можно запускать приложения для тестирования и просматривать их поведение в реальном времени.
4. AWS Device Farm Android-device - это сервис облачного тестирования приложений, разработанных для платформы Android.
...
1. IoT Device Management IndexData — это управляемый сервис, предоставляемый компанией IndexData, для управления и мониторинга устройств интернета вещей (IoT).
...
1. AWS Backup Early Delete-Cold - это функциональность в сервисе AWS Backup, которая позволяет удалить «холодные» резервные копии раньше заданного срока. «Холодные» резервные копии - это данные, которые хранятся на длительное время и, как правило, не требуются для восстановления операционных систем или приложений в ближайшем будущем.
2. AWS Backup Restore-Cold - это сервис восстановления данных AWS Backup, который позволяет восстанавливать резервные копии данных, хранящихся в холодном хранилище AWS Glacier. Холодное хранилище AWS Glacier - это служба долгосрочного хранения данных в облаке, предлагающая низкую стоимость хранения взамен более длительного времени доступа к данным.
3. AWS Backup Cold-Storage - это услуга хранения данных на прохладном хранилище, предлагаемая Amazon Web Services (AWS).
4. AWS Backup Restore-Warm - это одна из опций в службе AWS Backup, которая предоставляет возможность восстановления данных из резервных копий (бэкапов) в более быстрое хранилище, называемое "теплым хранилищем". Это предоставляет более быстрый доступ к данным после восстановления, чем в случае использования "холодного хранилища".
